...
class Editor extends React.Component {
  ...
  render() {
    ...
    let autoFields = null;
    if (user.has('email')) {
      const userInfo = accounts.filter(account => account.get('email') === user.get('email'));
      if (userInfo) {
        autoFields = userInfo.get(0).get('auto_fields');
      } else {
        console.error(`There is no information about the user: ${user.get('email')}`);
      }
    }
